...
def main(**kwargs):
    ...
    # policy.policy_grad = policy_grad
    # policy.value_grad = value_grad
    # with open('{0}/best_policy.json'.format(outdir), 'w') as f:
    #     json.dump(policy_grad, f)
    # with open('{0}/best_value.json'.format(outdir), 'w') as f:
    #     json.dump(value_grad, f)
    saver = tf.train.Saver()
    saver.save(sess, '{0}/best_sess.ckpt'.format(outdir))
    # saver.save(policy_grad, '{0}/best_policy'.format(outdir))
    # saver.save(value_grad, '{0}/best_value'.format(outdir))
