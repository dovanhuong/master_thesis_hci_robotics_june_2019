...
def write_csv(data):
    with open('Training_ddpg_result_0.csv', 'a') as out_file:
...
class ActionNoise:
    ...
    def __init__(self, action_dim, mu=0, theta=0.0, sigma=0.0): # action_dim, mu=0, theta=0.15, sigma=0.2
...
class Actor(nn.Module):
    ...
    def forward(self, state):
        ...
        if state.shape == torch.Size([2]):
            action[0] = torch.sigmoid(action[0]) * self.action_limit_l1
            action[1] = torch.sigmoid(action[1]) * self.action_limit_l2
...
ACTION_THETA1_MIN = -0.0500 #0.08
ACTION_THETA2_MIN = -0.0500

ACTION_L1_MAX = 0.35000   # 0.35
ACTION_L2_MAX = 0.2500  # 0.2
ACTION_THETA1_MAX = 0.0500
ACTION_THETA2_MAX = 0.0500
...
if __name__ == '__main__':
    ...
    past_action = np.array([0.30, 0.15, -0.05, -0.05])
    csvData = []
    num_success = 0

    with open('Training_ddpg_result_0.csv', 'a') as csvFile:
        writer = csv.writer(csvFile)
        csvData = [['action_l1', 'action_l2', 'action_theta1', 'action_theta2','initial_position_x', 'initial_position_y', 'achieved_position_x', 'achieved_position_y','reward',
                    'success']]
    ...
    for ep in range(MAX_EPISODES):
        ...
        for step in range(MAX_STEPS):
            ...
            if is_training:
                ...
                #print('action bugging', action[0], action[1], action[2], action[3])
                action[0] = np.random.uniform(ACTION_L1_MIN, ACTION_L1_MAX)
                action[1] = np.random.uniform(ACTION_L2_MIN, ACTION_L2_MAX)
                action[2] = np.random.uniform(ACTION_THETA1_MIN, ACTION_THETA1_MAX)
                action[3] = np.random.uniform(ACTION_THETA2_MIN, ACTION_THETA2_MAX)
                var_l1 = action[0]
                var_l2 = action[1]
                var_theta1 = action[2]
                var_theta2 = action[3]
                ...
                #action[0] = np.clip(np.random.uniform(ACTION_L1_MIN,ACTION_L1_MAX,100))
                #action[1] = np.clip(np.random.uniform(ACTION_L2_MIN, ACTION_L2_MAX,100))
                #action[2] = np.clip(np.random.uniform(ACTION_THETA1_MIN,ACTION_THETA1_MAX,100))
                #action[3] = np.clip(np.random.uniform(ACTION_THETA2_MIN, ACTION_THETA2_MAX, 100))
                ...
                #env.step(action)
                print('action in training', action[0], action[1], action[2], action[3])

            if not is_training:
                action = trainer.get_exploitation_action(state)

            next_state, distance, reward, done, success, info = env.step(action)
            ...
            with open('Training_ddpg_result_0.csv', 'a') as csvFile:
                writer = csv.writer(csvFile)
                csvData = [[str(action[0]), str(action[1]), str(action[2]), str(action[3]),str(initialposition_x), str(initialposition_y), str(achieved_position_x), str(achieved_position_y),str(reward), str(success)]]
                writer.writerows(csvData)
