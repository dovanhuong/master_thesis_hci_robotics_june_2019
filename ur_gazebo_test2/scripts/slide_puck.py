...
g_max_x = 0.0
g_max_y = 0.0
timestep_limit_per_episode = 1 #1
...
class UR5SlidePuckEnv(ur5_env.UR5Env):
    def __init__(self):
        ...
        setattr(self.param.init_obj_center, 'x', 0.4) #used was 0.45.0.41
        ...
        self.distance_threshold = 0.10  #0.1
    ...
    def _set_init_obj_pose(self):
        ...
        xx = x_pixel
        yy = y_pixel
        self.param.obj_center.x = random.choice(xx)
        self.param.obj_center.y = random.choice(yy)
        #self.param.obj_center.x = 0.35
        #self.param.obj_center.y = -0.15

        #self.param.obj_center.x = np.random.uniform(self.param.init_obj_center.x - self.param.init_obj_width/2,self.param.init_obj_center.x + self.param.init_obj_width/2)
        #self.param.obj_center.y = np.random.uniform(self.param.init_obj_center.y - self.param.init_obj_width / 2,self.param.init_obj_center.y + self.param.init_obj_width / 2)
        #self.param.obj_center.x = 0.4468178136757881
        #self.param.obj_center.y = -0.13289404771333083
    ...
    def _set_action(self, action):
        ...
        #print('huong check joint position',ordered_joint_position)
        x = np.identity(4)
        ...
        #print('move joint to angle blocking', self.move_joints_to_angle_blocking(joint_list,2.0))
        ...
        #print('\nfinal joint len', final_joint.__len__())
        ...
        #print('\nmove joint to angle blocking', self.move_joints_to_angle_blocking(joint_list,timestep,velocities))
