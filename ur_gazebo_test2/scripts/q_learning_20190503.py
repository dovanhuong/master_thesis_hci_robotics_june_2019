...
if __name__ == '__main__':
    ...
    with open('./result/Training_result_8.csv', 'a') as csvFile:
    ...
    with open('./result/Testing_result_8.csv', 'a') as csvFile:
        writer = csv.writer(csvFile)
        csvData = [['action_l1', 'action_l2', 'action_theta1', 'action_theta2','initial_position_x', 'initial_position_y', 'achieved_position_x', 'achieved_position_y','reward',
                    'success', 'success_rate']]
        writer.writerows(csvData)
        ...
        for ep in range(MAX_EPISODES):
            ...
            with open('./result/Training_result_8.csv', 'a') as csvFile:
                ...
                csvData = [[str(action[0]), str(action[1]), str(action[2]), str(action[3]), str(initialposition_x),
                            str(initialposition_y), str(achieved_position_x), str(achieved_position_y),str(reward), str(success), str(now_success_rate)]]
                writer.writerows(csvData)
        ...
        for ep in range(MAX_EPISODES):
            ...
            if (len(state_good) >=1):
                for i in range(len(state_good) - 1):
                    ...
                    with open('./result/Testing_result_9.csv', 'a') as csvFile:
                        writer = csv.writer(csvFile)
                        csvData = [
                            [str(action[0]), str(action[1]), str(action[2]), str(action[3]), str(initialposition_x),
                             str(initialposition_y), str(achieved_position_x), str(achieved_position_y),str(reward), str(success),
                             str(now_success_rate)]]
                        writer.writerows(csvData)
