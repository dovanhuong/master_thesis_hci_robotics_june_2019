...
void Listener::callback(const std_msgs::String::ConstPtr& msg) {
    memset(topic_message, 0, BUF_LEN);
...
*/
	...
	while(ros::ok())
	{
	    ROS_INFO("[Client] The client is running.");
        bzero(buffer,256);
        n = read(sockfd,buffer,255);
        if (n < 0)
            error("ERROR reading from socket");
	    ROS_INFO("[Client] I receive the message of %s", buffer);
	    stringToCommand(buffer, command);
